...
$('#editar-perfil').on('click', function () {
    ...
    var ellink=$("#link-volver-more").attr("href");
    
    $("#link-volver-more").attr("href",ellink+"$('#titulo-usuario').show();");
...
function muestramiqr() {
    var usu=$('input[name=email]').val();
    var h=(screen.height/2)-175;
    var w=(screen.width/2)-145;
    
        
    qrcode = new QRCode("miqrcode", {
        text: usu,
        width: 250,
        height: 250,
        colorDark : "#000000",
    ...
    $('#app').hide();
    $("#miqrcode-window").show();
    $("#miqrcode-window").css('top',h+'px');
    $("#miqrcode-window").css('left',w+'px');
}

function cierramiqr(){
    $("#miqrcode-window").hide();
    $("#miqrcode").html('');
    $('#app').show();
    qrcode.clear();
}

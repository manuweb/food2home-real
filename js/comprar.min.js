...
function checkout_2(){
    ...
    var txt_1='<div id="usuario-compras">';  
...
function cierrasesioncompra(){
    ...
    window.localStorage.setItem("username",''); 
    window.localStorage.setItem("mi_monedero",encripta('0')); window.localStorage.setItem("porcentaje_fidelizacion",encripta('0')); 
    window.localStorage.setItem("user",''); 
    $('#login-inicio').show();
    $('#usu-inicio').hide(); 
    ...
    $.ajax({
        ...
        success: function(data){ 
            ...
            if (obj.valid==true){
                ...
                //console.log('Monedero:'+obj.monedero);
...
function nuevoUsuCompra() {
    ...
    //console.log('estadopass:'+estadopass);
    if (estadopass='none'){
        pass='';
    }
...
function checkout_3(){
    ...
    /*
    console.log('Horas envio');
    console.log(horario);
    console.log('Horas cocina');
    ...
    */
    ...
    $.ajax({
        url: server,
        data:{horario:disponiblereparto, horariococina: disponiblecocina},
        method: "post",
        dataType:"json",
        success: function(data){ 
            var obj=Object(data);
            if (obj.valid==true){
                ...
                */
                txt=txt+'<div id="metodo-de-envio">'+
                    '<p>Seleccione opción de envío:</p>';
                var checked='';
                if (hay_cocina){
                ...
                if (hay_reparto){
                    if (disponiblereparto.length!=0){
                        ...
                        txt+='<p><label class="radio"><input type="radio" onclick="cambiametodoenvio();" name="metodo_envio" value="1" /><i class="icon-radio" '+checked+'></i></label> Entrega en mi domicilio</p>';
                    }
                }
                ...
                var entrega='Seleccione hora de recogida: <i class="f7-icons size-22">hand_point_right_fill</i> '+
...
function eligedomicilio(){
    ...
    var txt_pre=txt_envio+'<br>Hora:<b>'+hora_envio+'</b>'
    ...
    if (tipo_envio==1){
        ...
        if (window.localStorage.getItem("username")!=""){
            // registrado
           console.log('usuario:'+desencripta(window.localStorage.getItem("user"))); buscadirecciones_compra(desencripta(window.localStorage.getItem("user")));
...
function buscadirecciones_compra(id){
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            var txt_pre=txt_envio+'<br>Hora:<b>'+hora_envio+'</b>';
            $('#checkout-page').html($('#checkout-page').html()+txt_pre+txt)
        },
        error: function (xhr, ajaxOptions, thrownError) {
            console.log(xhr.status);
            console.log(thrownError);
...
function checkout_4(){
    ...
    var txt_pre=txt_envio+'<br>Hora:<b>'+hora_envio+'</b><br>'
    $('#icon-back-checkout').show(); 
    ...
    var txt2='<p>¿Desea comentarnos algo?<span id="caracteres" style="float:right;"></span></p>'+
        ...
        if (fidelizacion==1){
            ...
            if (parseFloat(monedero_actual) >0) {
                ...
                txt2+='<p style="display: flex;"><label class="checkbox"><input type="checkbox" onclick="cambiausomonedero(this);" id="monedero" /><i class="icon-checkbox"></i></label>&nbsp;&nbsp;<i class="f7-icons">giftcard</i>&nbsp;&nbsp;usar <b><span id="valor-monedero-usar">'+parseFloat(monedero_actual).toFixed(2) +'</span>€ </b>de mi tarjeta regalo</p>';
            }   
        }
    ...
    $('#checkout-page').html(txt+txt_pre+txt_cart+txt2+txt_botoncancelar);
...
function cambiametodoenvio(){
    ...
    if (envio=='1') {
        if (disponiblereparto.length==0){
            muestraMensaje('No tenemos horario para hoy',titulo='',subtitulo='Horario reparto');
        ...
        for (x=0;x<disponiblereparto.length;x++){
            ...
            opciones+='<option value="'+disponiblereparto[x]+'">'+disponiblereparto[x]+txt_hora_cortesia+'</option>';
        }
    }
    else {
        texto='de recogida';
        ...
        $('#buy-cart').html(cart); 
        if (disponiblecocina.length==0){
            muestraMensaje('No tenemos horario para hoy',titulo='',subtitulo='Horario recogida');
            $('#compra-paso-2').prop('disabled', true);
        ...
        var txt_hora_cortesia='';
         for (x=0;x<disponiblecocina.length;x++){
             if (cortesia>0){ 
                hora=parseInt(disponiblecocina[x].substr(0,2));
                minutos=parseInt(disponiblecocina[x].substr(3,2));
                minutos+=cortesia;
                if (minutos>=60){
                    minutos-=cortesia;
            ...
            opciones+='<option value="'+disponiblecocina[x]+'">'+disponiblecocina[x]+txt_hora_cortesia+'</option>';
        }       
    }

    var entrega='Seleccione hora '+texto+': <i class="f7-icons size-22">hand_point_right_fill</i> '+
        ...
        success: function(data){
            ...
            if (obj.valid==true){
                ...
                $.ajax({
                    type: "POST",
                    url: server,
                    data: {cliente:cliente,canal:canal,tratamiento:tratamiento,nombre:nombre,apellidos:apellidos, telefono:telefono, email:email,carrito:carrito, publicidad:publicidad, domicilio:carrito_domicilio, envio:envio,tarjeta:tarjeta, hora:hora, subtotal:parseFloat(carritoSubtotal), portes:parseFloat(carrito_domicilio['envio']), descuento:parseFloat(descuento), tipo_descuento:tipo_descuento, monedero:parseFloat(monedero_a_usar),total:parseFloat(tot), importe_fidelizacion:parseFloat(importe_fidelizacion), cupon:cupon, comentario:comentario, codigocupon:codigocupon,  idcupon:idcupon},
                    dataType:"json",
                    success: function(data){
                        var obj=Object(data);
                        if (obj.valid==true){
                ...
                app.dialog.alert('La hora <b>'+hora+'</b> ya no está disponible.<br>Seleccione otra hora','Atención');
...
function finalizaPedido(idpedido,pedido,modo){
    ...
    */
    if(modo!=idRedsys){
        ...
        $.ajax({
            url: server,
            data:{idpedido:idpedido,tipo:'pedido',numero:pedido},
            method: "post",
            dataType:"json",
            success: function(data){ 
                var obj=Object(data);
            ...
            success: function(data){ 
                ...
                if (obj.valid==true){
                    ...
                    if (idRevo>0){
                        ...
                        actualizapedidoRevo(idpedido,idRevo);
...
function actualizapedidoRevo(idpedido,idrevo){
    //console.log('Actualizando ..');
...
var txt_compra_form='<form class="list" id="my-form-usu-compra" >'+
    '<p class="block-title" id="titulo-nuevo-usu-compras">Nuevo usuario</p>'+
                ...
                '<li>'+
                    '<div class="item-content item-input">'+
                        '<div class="item-inner">'+
                            ...
                            '<div class="item-input-wrap">'+
                                '<input type="text" name="telefono" placeholder="Teléfono" required validate/>'+
                            '</div>'+
                        '</div>'+
                    '</div>'+
                '</li>'+
            ...
            '<ul>'+
                ...
                '<li>'+
                   '<label class="item-checkbox item-content">'+
                        '<input type="checkbox" name="privacidad-checkbox" value="1" />'+
                        '<i class="icon icon-checkbox"></i>'+
                        '<div class="item-inner">'+
                ...
                '</li>'+
            '</ul>'+
            '<div class="text-align-center" style="width:50%;margin:auto;height: 35px;"><a class="button button-fill button-round" onclick="nuevoUsuCompra();" href="#">Continuar</a></div>' +
        '</form>';

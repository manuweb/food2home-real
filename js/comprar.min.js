...
function checkout_1(){
    ...
    */
    ...
    if (soloRegalo>0 &&(normales+menus)==0){
        llevabolsa='no';
    }
    else {
        if (idBolsa!=0){
    ...
    (carritoSubtotal).toFixed(2)+'€</b></span></span><br><br>';
        ...
        var cart='<span><b>Total (impuestos inc.)</b><span style="float:right;font-size: 1.3em;"><b>'+
    (carritoSubtotal+precioBolsa).toFixed(2)+'€</b></span></span><br><br>';
    }
...
function ponehorasdehoy(fecha) {
    //console.log('fecha_pedido:'+fecha_pedido);
    var esfestivo=festivos.includes(fecha_pedido);
    if (esfestivo){
        muestraMensaje('No hay horarios para '+fecha_pedido,titulo='',subtitulo='Horarios');
        checkout_2();
        return;
    }
    ...
    //console.log('Busco pedidos:'+diaCastellano(d.getDate()) + "/" + mesCastellano(d.getMonth()) + "/" + d.getFullYear());
        ...
        disponiblereparto=horario;
        
        disponiblecocina=horariococina;
...
function checkout_3(){
    ...
    if (esfestivo){
        muestraMensaje('No hay horarios para '+fecha_pedido,titulo='',subtitulo='Horarios');
        $('#button-envio-2').attr('disabled',true);
        $('#button-envio-1').attr('disabled',true);
    }
    else {
        $('#button-envio-2').attr('disabled',false);
        $('#button-envio-1').attr('disabled',false);
    }
    
    //console.log('fecha_pedido:'+fecha_pedido);
    
    if (solotr=='si'){
        //console.log('es solo tr');
        checkout_4();
        tipo_envio=2
        var hor=d.getHours();
        ...
        //console.log('Hoy:'+fecha_pedido);
        //console.log('Hora:'+hora_envio);
...
function llenafechaspedido(dias){
    ...
    for (var j=0;j<dias;j++){
        ...
        if (!esfestivo){
        txt+='<button class="button-fecha-reparto button button-large '+primero+'" data-fecha="'+losdias[j]+'" style="line-height: 18px;" onclick="'+elonclick+'" '+desabilitado+'><p><b>'+txt_prim+'</b><br><span style="font-size:14px;">'+losdias[j]+'</span></p></button>';}
...
function cambiaDiaEnvioCalendar(){
    ...
    var disables=new Array();
    disables.push({from: minD,to: inicio});
    disables.push({from: fin,to: maxD});
    
    // 01/34/6789
    
    for (var j=0;j<festivos.length;j++){
        //var diafestivo= new Date (parseInt(festivos[j].substr(6,4)), parseInt(festivos[j].substr(2,2)), parseInt(festivos[j].substr(0,2)));
        var arregloFecha = festivos[j].split("/");
        var anio = arregloFecha[2];
        var mes = arregloFecha[1] - 1;
        var dia = arregloFecha[0];

        var eldia = new Date(anio, mes, dia); // fecha será de tipo Date
...
function ponelashoras(hay_reparto,hay_cocina){
    ...
    var metodo=1;
    if (dias_vista>1 && tipo_seleccion_horas>0){
...
function ponelashorassegundia(metodo,hay_reparto,hay_cocina){
    ...
    if (tipo_seleccion_horas==0){
        ...
        var lashorasD=new Array();
        var lahora=hour + ":" + minute;
        var primera=lahora;
        var ultima=lahora;
        
        now = new Date(numberOfMlSeconds + (tiempoenvio*60000));
        
        if (metodo==1){
            //domicilio
            lashorasD=disponiblereparto;
        }
        else {
            lashorasD=disponiblecocina;        
        }
        
        var lashorasN=new Array();
        var h=0;
        console.log('lahora->'+lahora);
        for (x=0;x<lashorasD.length;x++){
            if (lahora<=lashorasD[x]){
                lashorasN[h]=lashorasD[x];
                h++;
            }
        }

        primera=lashorasN[0];
        ultima=lashorasN[lashorasN.length-1];
        hour = now.getHours();
        ...
        console.log(lashorasN);


        if (lahora>ultima || primera==undefined){
            //console.log('NO Disponible');
            app.dialog.alert('No hay horarios para hoy');
            // no continuar
            return;
        }
        if (lahora<=primera){
            
            app.dialog.alert('Restaurante cerrado<br>Entrega programada para las '+primera);
            console.log('NO abierto aun');
            lahora=primera;
        }
        hora_envio=lahora;
        if (metodo==1){
            //domicilio
            $('#hora-reparto').val(hora_envio); 
        }
        else {
            $('#hora-recogida').val(hora_envio); 
        }
...
function cambiametodoenviobut(elem,hay_reparto,hay_cocina){
    ...
    //console.log('tipo_envio:'+tipo_envio);
    //ponelashoras(hay_reparto,hay_cocina);
    if (tipo_envio==1 && parseFloat(carritoSubtotal)<parseFloat(pedidominimo)){
...
function checkout_4(){
        ...
        if (fidelizacion==1){
            ...
            if (parseFloat(monedero_actual) >0) {
                if (llevabolsa=='no'){
                    ...
                    if (parseFloat(monedero_actual) >(carritoSubtotal+parseFloat(carrito_domicilio['envio']+precioBolsa)-descuento)) {
                        monedero_actual=carritoSubtotal+parseFloat(carrito_domicilio['envio']+precioBolsa)-descuento;
    ...
    txt2+='<div id="los-metodos-pago">';
    /*
        console.log('solotr:'+solotr);
            console.log('idRedsys:'+idRedsys);
            console.log('Bizum:'+bizum);
            */
        for (x=0;x<metodosdepago.length;x++){
            ...
            if (metodosdepago[x]['id']==1){
                ...
                img='&nbsp;&nbsp;<img src="img/cash.png" width="28px" height="auto"  align="absmiddle">&nbsp;&nbsp;';
            ...
            if (solotr=='si' && metodosdepago[x]['idrevo']==2){
                txt2+='';
            }
            else {
                //console.log('id:'+metodosdepago[x]['id']);
                txt2+='<p style="display: flex;"><label class="radio"><input type="radio" name="metodo_pago" id="metodo_pago" value="'+metodosdepago[x]['idrevo']+'" data-tipo="nobizum" '+chk+' onclick="cambiametodopago(this)";/><i class="icon-radio"></i></label> '+img+'  '+metodosdepago[x]['nombre']+'</p>';
                
                if (metodosdepago[x]['idrevo']==idRedsys && bizum==1){
                    //console.log('hay bizum');
                    txt2+='<p style="display: flex;"><label class="radio"><input type="radio" name="metodo_pago" id="metodo_pago" value="'+idRedsys+'" data-tipo="bizum" onclick="cambiametodopago(this)";/><i class="icon-radio"></i></label> &nbsp;&nbsp;<img src="img/bizum.png" width="28px" height="auto"  align="absmiddle">&nbsp;&nbsp;  Bizum</p>';
...
descuento=trunc (descuento, 2);
    ...
    cantidad+=' <span onclick="muestraocultacarrito();" id="detalles-carrito" >(<span class="text-color-primary">mostrar detalles <i class="f7-icons" style="font-size:14px;">chevron_down</i></span>)</span>';
     ...
     var cart='<span>'+cantidad+'<span style="float:right;">';
    if(llevabolsa=='no'){
        cart+=carritoSubtotal.toFixed(2);
        
    }
    else {
        cart+=(carritoSubtotal+precioBolsa).toFixed(2);
    }
    cart+='€</span></span><span style="display:none;" id="carrito-oculto"><br><br>'+mostrarcarrito()+'</span><br>'+
    ...
    //console.log('llevabolsa:'+llevabolsa);
    if(llevabolsa=='no'){
        cart+=(carritoSubtotal+parseFloat(carrito_domicilio['envio'])-descuento-monedero_a_usar).toFixed(2)+'€</b></span></span>';
    }
    else {
        cart+=(carritoSubtotal+parseFloat(carrito_domicilio['envio'])-descuento-monedero_a_usar+precioBolsa).toFixed(2)+'€</b></span></span>';
    }
...
function mostrarcarrito(){
    ...
    for (var x=0;x<carrito.length;x++) {
        ...
        if (carrito[x]['menu']==5){
            ...
            carrito[x]['nombreT']+'<BR>'+carrito[x]['emailT']+'</span>'
        }
    ...
    if (soloRegalo>0 && (normales+menus)==0 ){
        //console.log('solo regalo');
        solotr='si';
    }
    else {
        solotr='no';
        //console.log('poner bolsa');
        if (idBolsa!=0){
           llevabolsa='si';
           var txt_bolsa='Real Decreto 293/2018';
            //console.log('hay que poner bolsas');
            txt+='<hr>  <div style="align-items: center;display: flex;">'+
                '<div class="col-35" style="width:35%;"></div>'+
                '<div class="col-65 text-aling-left" style="width:65%;">'+productoBolsa+'<br><span style="font-size:.7em;">'+
                txt_bolsa+'</span><div style="text-align: right;"><span style="font-size:.9em;">'+
                1+' x </span><span class="text-color-primary" style="font-size:1.2em;">'+precioBolsa.toFixed(2)+'€</span></div></div>'+    
            '</div>';
        }
    }
    return txt+'<hr>';
...
//console.log('Monedero:'+$('#valor-monedero-usar').html());
    if (activo){
    ...
    if (tot.toFixed(2)=='0.00'){
            
        $('#los-metodos-pago').hide();
    }
    else {
        $('#los-metodos-pago').show();
    }
}
   ...
   if (fidelizacion=='1'){
    ...
    if (tipo_seleccion_horas==0){
        ...
        $.ajax({
            type: "POST",
            url: server,
            data: {cliente:cliente,canal:canal,tratamiento:tratamiento,nombre:nombre,apellidos:apellidos, telefono:telefono, email:email,carrito:carrito, publicidad:publicidad, domicilio:carrito_domicilio, envio:envio,tarjeta:tarjeta, hora:hora,dia:dia, subtotal:parseFloat(carritoSubtotal), portes:parseFloat(carrito_domicilio['envio']), descuento:parseFloat(descuento), tipo_descuento:tipo_descuento, monedero:parseFloat(monedero_a_usar),total:parseFloat(tot), importe_fidelizacion:parseFloat(importe_fidelizacion), cupon:cupon, comentario:comentario, codigocupon:codigocupon,  idcupon:idcupon,llevabolsa:llevabolsa,idBolsa:idBolsa,precioBolsa:precioBolsa,productoBolsa:productoBolsa},
            dataType:"json",
            success: function(data){
                var obj=Object(data);
                if (obj.valid==true){
                    ...
                    if (parseInt(tarjeta)==parseInt(idRedsys)) {       //var tot=carritoSubtotal;
                    if (llevabolsa=='si'){
                        total=parseFloat(total)+parseFloat(precioBolsa); 
                        total=total.toFixed(2);
                    }

                        document.all.iframe_tpv.src = servidor+'includes/tpv.php?importe='+total+'&order='+obj.order+'&idpedido='+obj.idpedido+'&esbizum='+esbizum;
                    ...
                    if (obj.msg=='error datos') {

                        app.dialog.alert('No hemos recibidos sus datos personales y no podemos procesar el pedido.<br>Si el error continúa contacte con nosotros.','Error');
                    }
                    else {
        ...
        $.ajax({
            type: "POST",
            url: server,
            data: {envio:envio, hora:hora, dia:dia,disponiblereparto:disponiblereparto,disponiblecocina:disponiblecocina,solotr:solotr},
            dataType:"json",
            success: function(data){
                var obj=Object(data);
                if (obj.valid==true){
...
function finalizaPedido(idpedido,pedido,modo){
    ...
    */
    ...
    if(integracion==1){
        ...
        $.ajax({
            url: server,
            data:{idpedido:idpedido,numero:pedido,copias:copiasTickets},
            method: "post",
            dataType:"json",
            success: function(data){ 
                var obj=Object(data);

...
var app = new Framework7({
  ...
  on: {
	...
	var thisurl=window.location.href;
        var estaurl=thisurl.split("?");
	var instagram = getParameterByName('fbclid');
	if (instagram!=''){
	    //alert(location);
	    window.open(estaurl[0], "_self");    
	}
	else {
...
if( window.navigator.standalone) {
  // it's in standalone mode
  //tool-bar-menu-icon
    $('#tool-bar-menu-icon').css('position','absolute');
    $('#tool-bar-menu-icon').css('bottom','-20px');
}
...
function leeEstaoWeb() {
    ...
    $.ajax({
        ...
        success: function(data){
            var obj=Object(data);
            if (obj.valid==true){
                incluyeQuiosco=obj.incluyeQuiosco;
                idBolsa=obj.idBolsa;
                productoBolsa=obj.productoBolsa;
                idRedsys=obj.idRedsys;
                bizum=obj.bizum;
                   precioBolsa=parseFloat(obj.precioBolsa); 
                norepartomensaje=obj.norepartomensaje;
                portesgratis=obj.portesgratis;
               multiIdiomas=obj.multiIdiomas; importeportesgratis=obj.importeportesgratis;
               ...
               //console.log('delivery:'+delivery); 
                ...
                if (obj.on==0){
                   //Web operativa
                    ...
                    copiasTickets=obj.copiasTickets;
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if (obj.valid==true){
                ...
                destacadosinicio();
		// ver si existe destino
                if (window.localStorage.getItem("navegara")!=null) {
                    var nav=window.localStorage.getItem("navegara");
...
function clicenImgInicio(e) {
    ...
    if(elprod!=""){
        var catprod=elprod.split('===');
        navegar('#view-catalog');
        if (catprod[0]=='P'){
            muestraproductoinicio(catprod[1]);
        }
        else {
            muestracategoriainicio(catprod[1]);
        }
